// Unit tests for the InfinitagNow protocol library.
...
void test_target_blob_roundtrip() {
  ...
  TEST_ASSERT_EQUAL_UINT8(7, out.target_id);
  TEST_ASSERT_EQUAL_UINT8(2, out.station_id);
...
void test_target_blob_endianness() {
  // hit_time_ms = 0x1234 must serialize little-endian at offsets 3..4
  TargetConfig in;
  in.hit_time_ms = 0x1234;
  uint8_t blob[TARGET_BLOB_SIZE];
  encodeTargetConfig(in, blob);
  TEST_ASSERT_EQUAL_HEX8(0x34, blob[3]);
  TEST_ASSERT_EQUAL_HEX8(0x12, blob[4]);
}
...
void test_discover_reply_roundtrip() {
  ...
  TEST_ASSERT_EQUAL_UINT8(1, out.fw_minor);
  ...
  TEST_ASSERT_EQUAL_UINT8(5, tcOut.target_id);
}
